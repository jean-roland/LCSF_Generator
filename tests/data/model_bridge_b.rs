//! Lcsf abstraction layer (B) for protocol: Test
//!
//! This file has been auto-generated by LCSF Generator v1.4
//! It shouldn't be edited manually

use crate::lcsf_lib::lcsf_validator;
use crate::lcsf_prot::protocol_test_b;
...
use protocol_test_b::Cc1AttPayload;
use protocol_test_b::Cc3AttPayload;
use protocol_test_b::Cc4AttPayload;
use protocol_test_b::Cc4AttCa1Payload;
use protocol_test_b::Cc4AttCa2Payload;
use protocol_test_b::Ca2AttCa3Payload;
use protocol_test_b::Ca3AttCa4Payload;
use protocol_test_b::Cc6AttPayload;
use protocol_test_b::Cc6AttCa9Payload;
use protocol_test_b::Cc6AttCa10Payload;
use protocol_test_b::Ca10AttCa11Payload;
use protocol_test_b::Ca11AttCa12Payload;
use protocol_test_b::CmdEnum;
use protocol_test_b::CmdPayload;
...
fn cc1_get_data(att_arr: &[LcsfValidAtt]) -> CmdPayload {
    let mut cc1_payload = Cc1AttPayload {
        sa1: 0,
        sa2: 0,
        sa3: 0,
        sa4: Vec::new(),
    ...
    if let LcsfValidAttPayload::Data(data) = &att_iter.next().unwrap().payload {
        cc1_payload.sa1 = u8::from_le_bytes(data.as_slice().try_into().unwrap());
    }
    // Retrieve data of attribute sa2
    if let LcsfValidAttPayload::Data(data) = &att_iter.next().unwrap().payload {
        cc1_payload.sa2 = lcsf_validator::vle_decode(data) as u16;
    }
    // Retrieve data of attribute sa3
    if let LcsfValidAttPayload::Data(data) = &att_iter.next().unwrap().payload {
        cc1_payload.sa3 = lcsf_validator::vle_decode(data) as u32;
    }
    // Retrieve data of attribute sa4
    if let LcsfValidAttPayload::Data(data) = &att_iter.next().unwrap().payload {
        cc1_payload.sa4 = data.clone();
    }
    // Retrieve data of attribute sa5
    if let LcsfValidAttPayload::Data(data) = &att_iter.next().unwrap().payload {
        cc1_payload.sa5 = CString::from_vec_with_nul(data.clone()).unwrap();
    }
    // Retrieve data of attribute sa6
    if let LcsfValidAttPayload::Data(data) = &att_iter.next().unwrap().payload && !data.is_empty() {
        cc1_payload.is_sa6_here = true;
        cc1_payload.sa6 = u8::from_le_bytes(data.as_slice().try_into().unwrap());
    }
    // Retrieve data of attribute sa7
    if let LcsfValidAttPayload::Data(data) = &att_iter.next().unwrap().payload && !data.is_empty() {
        cc1_payload.is_sa7_here = true;
        cc1_payload.sa7 = lcsf_validator::vle_decode(data) as u16;
    }
    // Retrieve data of attribute sa8
    if let LcsfValidAttPayload::Data(data) = &att_iter.next().unwrap().payload && !data.is_empty() {
        cc1_payload.is_sa8_here = true;
        cc1_payload.sa8 = lcsf_validator::vle_decode(data) as u32;
    }
    // Retrieve data of attribute sa9
    if let LcsfValidAttPayload::Data(data) = &att_iter.next().unwrap().payload && !data.is_empty() {
        cc1_payload.is_sa9_here = true;
        cc1_payload.sa9 = data.clone();
    }
    // Retrieve data of attribute sa10
    if let LcsfValidAttPayload::Data(data) = &att_iter.next().unwrap().payload && !data.is_empty() {
        cc1_payload.is_sa10_here = true;
        cc1_payload.sa10 = CString::from_vec_with_nul(data.clone()).unwrap();
    }
    // Retrieve data of attribute sa11
    if let LcsfValidAttPayload::Data(data) = &att_iter.next().unwrap().payload {
        cc1_payload.sa11 = lcsf_validator::vle_decode(data);
    }
    // Retrieve data of attribute sa12
    if let LcsfValidAttPayload::Data(data) = &att_iter.next().unwrap().payload {
        cc1_payload.sa12 = f32::from_le_bytes(data.as_slice().try_into().unwrap());
    }
    // Retrieve data of attribute sa13
    if let LcsfValidAttPayload::Data(data) = &att_iter.next().unwrap().payload {
        cc1_payload.sa13 = f64::from_le_bytes(data.as_slice().try_into().unwrap());
    }
    CmdPayload::Cc1Payload(cc1_payload)
}
...
fn cc3_get_data(att_arr: &[LcsfValidAtt]) -> CmdPayload {
    ...
    // Retrieve data of attribute sa2
    if let LcsfValidAttPayload::Data(data) = &att_iter.next().unwrap().payload {
        cc3_payload.sa2 = lcsf_validator::vle_decode(data) as u16;
    }
    // Retrieve data of attribute sa3
    if let LcsfValidAttPayload::Data(data) = &att_iter.next().unwrap().payload {
...
fn cc4_get_data(att_arr: &[LcsfValidAtt]) -> CmdPayload {
    let mut cc4_payload = Cc4AttPayload {
        sa1: 0,
    ...
    // Retrieve data of attribute sa1
    if let LcsfValidAttPayload::Data(data) = &att_iter.next().unwrap().payload {
        cc4_payload.sa1 = u8::from_le_bytes(data.as_slice().try_into().unwrap());
    }
    ...
    if let LcsfValidAttPayload::SubattArr(subatt_arr) = &att_iter.next().unwrap().payload {
        ...
        if let LcsfValidAttPayload::Data(data) = &subatt_iter.next().unwrap().payload {
            cc4_payload.ca1_payload.sa1 = u8::from_le_bytes(data.as_slice().try_into().unwrap());
        }
        // Retrieve data of sub-attribute sa2
        if let LcsfValidAttPayload::Data(data) = &subatt_iter.next().unwrap().payload {
            cc4_payload.ca1_payload.sa2 = lcsf_validator::vle_decode(data) as u16;
        }
        // Retrieve data of sub-attribute sa3
        if let LcsfValidAttPayload::Data(data) = &subatt_iter.next().unwrap().payload && !data.is_empty() {
            cc4_payload.ca1_payload.is_sa3_here = true;
            cc4_payload.ca1_payload.sa3 = lcsf_validator::vle_decode(data) as u32;
        }
    }
    ...
    if let LcsfValidAttPayload::SubattArr(subatt_arr) = &att_iter.next().unwrap().payload && !subatt_arr.is_empty() {
        cc4_payload.is_ca2_here = true;
        // Get iterator
        let subatt_iter = &mut subatt_arr.iter();
        // Retrieve data of sub-attribute sa1
        if let LcsfValidAttPayload::Data(data) = &subatt_iter.next().unwrap().payload && !data.is_empty() {
            cc4_payload.ca2_payload.is_sa1_here = true;
            cc4_payload.ca2_payload.sa1 = u8::from_le_bytes(data.as_slice().try_into().unwrap());
        }
        ...
        if let LcsfValidAttPayload::SubattArr(subatt_arr) = &subatt_iter.next().unwrap().payload {
            ...
            // Retrieve data of sub-attribute sa1
            if let LcsfValidAttPayload::Data(data) = &subatt_iter.next().unwrap().payload && !data.is_empty() {
                cc4_payload.ca2_payload.ca3_payload.is_sa1_here = true;
                cc4_payload.ca2_payload.ca3_payload.sa1 = u8::from_le_bytes(data.as_slice().try_into().unwrap());
            }
            ...
            if let LcsfValidAttPayload::SubattArr(subatt_arr) = &subatt_iter.next().unwrap().payload {
                ...
                if let LcsfValidAttPayload::Data(data) = &subatt_iter.next().unwrap().payload {
                    cc4_payload.ca2_payload.ca3_payload.ca4_payload.sa4 = data.clone();
                }
            }
        }
    }
    CmdPayload::Cc4Payload(cc4_payload)
}
...
pub fn receive_cmd(valid_cmd: &LcsfValidCmd) -> (CmdEnum, CmdPayload) {
    ...
    let cmd_payload = match cmd_name {
        ...
        CmdEnum::Cc1 => cc1_get_data(&valid_cmd.att_arr),
        CmdEnum::Cc2 => CmdPayload::Empty,
        CmdEnum::Cc3 => cc3_get_data(&valid_cmd.att_arr),
        CmdEnum::Cc4 => cc4_get_data(&valid_cmd.att_arr),
        CmdEnum::Cc5 => CmdPayload::Empty,
...
fn cc2_fill_att(cmd_payload: &CmdPayload) -> Vec<LcsfValidAtt> {
    let mut att_arr: Vec<LcsfValidAtt> = Vec::new();
    if let CmdPayload::Cc2Payload(payload) = &cmd_payload {
        // Fill data of attribute sa1
        att_arr.push(LcsfValidAtt { payload: LcsfValidAttPayload::Data(lcsf_validator::vle_encode(payload.sa1 as u64)),});
        // Fill data of attribute sa2
        att_arr.push(LcsfValidAtt { payload: LcsfValidAttPayload::Data(lcsf_validator::vle_encode(payload.sa2 as u64)),});
        // Fill data of attribute sa3
        att_arr.push(LcsfValidAtt { payload: LcsfValidAttPayload::Data(lcsf_validator::vle_encode(payload.sa3 as u64)),});
        ...
        if payload.is_sa6_here {
            att_arr.push(LcsfValidAtt { payload: LcsfValidAttPayload::Data(lcsf_validator::vle_encode(payload.sa6 as u64)),});
        } else {
            att_arr.push(LcsfValidAtt { payload: LcsfValidAttPayload::Data(Vec::new()),});
        }
        // Fill data of attribute sa7
        if payload.is_sa7_here {
            att_arr.push(LcsfValidAtt { payload: LcsfValidAttPayload::Data(lcsf_validator::vle_encode(payload.sa7 as u64)),});
        } else {
            att_arr.push(LcsfValidAtt { payload: LcsfValidAttPayload::Data(Vec::new()),});
        }
        // Fill data of attribute sa8
        if payload.is_sa8_here {
            att_arr.push(LcsfValidAtt { payload: LcsfValidAttPayload::Data(lcsf_validator::vle_encode(payload.sa8 as u64)),});
        } else {
            att_arr.push(LcsfValidAtt { payload: LcsfValidAttPayload::Data(Vec::new()),});
        }
        ...
        att_arr.push(LcsfValidAtt { payload: LcsfValidAttPayload::Data(lcsf_validator::vle_encode(payload.sa11)),});
        // Fill data of attribute sa12
        att_arr.push(LcsfValidAtt { payload: LcsfValidAttPayload::Data(payload.sa12.to_le_bytes().to_vec()),});
        // Fill data of attribute sa13
        att_arr.push(LcsfValidAtt { payload: LcsfValidAttPayload::Data(payload.sa13.to_le_bytes().to_vec()),});
    }
    att_arr
}
...
fn cc5_fill_att(cmd_payload: &CmdPayload) -> Vec<LcsfValidAtt> {
    let mut att_arr: Vec<LcsfValidAtt> = Vec::new();
    if let CmdPayload::Cc5Payload(payload) = &cmd_payload {
        ...
        att_arr.push(LcsfValidAtt { payload: LcsfValidAttPayload::SubattArr(vec![
            ...
            LcsfValidAtt { payload: LcsfValidAttPayload::Data(lcsf_validator::vle_encode(payload.ca5_payload.sa1 as u64))},
            // Fill data of sub-attribute sa2
            LcsfValidAtt { payload: LcsfValidAttPayload::Data(lcsf_validator::vle_encode(payload.ca5_payload.sa2 as u64))},
            // Fill data of sub-attribute sa3
            LcsfValidAtt { payload: if payload.ca5_payload.is_sa3_here {
                LcsfValidAttPayload::Data(lcsf_validator::vle_encode(payload.ca5_payload.sa3 as u64))
            } else {
        ...
        if payload.is_ca6_here {
            att_arr.push(LcsfValidAtt { payload: LcsfValidAttPayload::SubattArr(vec![
                // Fill data of sub-attribute sa1
                LcsfValidAtt { payload: if payload.ca6_payload.is_sa1_here {
                    LcsfValidAttPayload::Data(lcsf_validator::vle_encode(payload.ca6_payload.sa1 as u64))
                } else {
                    LcsfValidAttPayload::Data(Vec::new())
                }},
                ...
                LcsfValidAtt { payload: LcsfValidAttPayload::SubattArr(vec![
                    // Fill data of sub-attribute sa1
                    LcsfValidAtt { payload: if payload.ca6_payload.ca7_payload.is_sa1_here {
                        LcsfValidAttPayload::Data(lcsf_validator::vle_encode(payload.ca6_payload.ca7_payload.sa1 as u64))
                    } else {
                        LcsfValidAttPayload::Data(Vec::new())
                    }},
                    ...
                    LcsfValidAtt { payload: LcsfValidAttPayload::SubattArr(vec![
                        ...
                        LcsfValidAtt { payload: LcsfValidAttPayload::Data(payload.ca6_payload.ca7_payload.ca8_payload.sa4.clone())},
                    ])},
                ])},
            ]),});
        } else {
...
fn cc6_fill_att(cmd_payload: &CmdPayload) -> Vec<LcsfValidAtt> {
    ...
    if let CmdPayload::Cc6Payload(payload) = &cmd_payload {
        ...
        att_arr.push(LcsfValidAtt { payload: LcsfValidAttPayload::SubattArr(vec![
            // Fill data of sub-attribute sa1
            LcsfValidAtt { payload: LcsfValidAttPayload::Data(lcsf_validator::vle_encode(payload.ca9_payload.sa1 as u64))},
            // Fill data of sub-attribute sa2
            LcsfValidAtt { payload: LcsfValidAttPayload::Data(lcsf_validator::vle_encode(payload.ca9_payload.sa2 as u64))},
            // Fill data of sub-attribute sa3
            LcsfValidAtt { payload: if payload.ca9_payload.is_sa3_here {
                LcsfValidAttPayload::Data(lcsf_validator::vle_encode(payload.ca9_payload.sa3 as u64))
            } else {
                LcsfValidAttPayload::Data(Vec::new())
            }},
        ]),});
        ...
        if payload.is_ca10_here {
            att_arr.push(LcsfValidAtt { payload: LcsfValidAttPayload::SubattArr(vec![
                ...
                LcsfValidAtt { payload: if payload.ca10_payload.is_sa1_here {
                    LcsfValidAttPayload::Data(lcsf_validator::vle_encode(payload.ca10_payload.sa1 as u64))
                } else {
                    LcsfValidAttPayload::Data(Vec::new())
                }},
                ...
                LcsfValidAtt { payload: LcsfValidAttPayload::SubattArr(vec![
                    ...
                    LcsfValidAtt { payload: if payload.ca10_payload.ca11_payload.is_sa1_here {
                        LcsfValidAttPayload::Data(lcsf_validator::vle_encode(payload.ca10_payload.ca11_payload.sa1 as u64))
                    } else {
                        LcsfValidAttPayload::Data(Vec::new())
                    }},
...
pub fn send_cmd(cmd_name: CmdEnum, cmd_payload: &CmdPayload) -> LcsfValidCmd {
    ...
    match cmd_name {
        ...
        CmdEnum::Sc3 => {},
        CmdEnum::Cc1 => {},
        CmdEnum::Cc2 => send_cmd.att_arr = cc2_fill_att(cmd_payload),
        CmdEnum::Cc3 => send_cmd.att_arr = cc3_fill_att(cmd_payload),
        CmdEnum::Cc4 => {},
        CmdEnum::Cc5 => send_cmd.att_arr = cc5_fill_att(cmd_payload),
